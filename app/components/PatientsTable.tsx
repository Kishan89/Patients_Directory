...
export default function PatientsTable() {
  ...
  const [sortBy, setSortBy] = useState<SortableField>('patient_id')
    ...
    <div className="px-8 pb-8">
      <div className="flex flex-col md:flex-row items-center gap-4 bg-slate-50 border rounded p-4">
        ...
        <div className="relative w-full md:w-auto" ref={filterRef}>
          <button
            ...
            className="w-full md:w-auto px-4 py-3 border rounded bg-white flex items-center justify-center gap-2"
          >
            <svg
              width="16"
              height="16"
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFilters((f) => ({ ...f, medical: e.target.value }))}
          ...
          <table className="min-w-full table-fixed">
            <thead className="text-sm text-slate-600">
              <tr>
                ...
                <th className="text-left py-3 pr-6 w-[180px]">Phone</th>
                <th className="text-left py-3 pr-6 w-[260px]">Email</th>
              </tr>
            </thead>
                  ...
                  <tr key={p.patient_id} className="border-t hover:bg-slate-50">
                    <td className="py-4 pr-6">
                      <div className="text-xs text-slate-500">
                        {String(p.patient_id).padStart(3, '0')}
                      </div>
                    </td>
                    <td className="py-4 pr-6">
                    ...
                    <td
                      className={`py-4 pr-6 ${
                        phoneIsNA ? 'text-red-600' : 'text-emerald-600 font-medium'
                      }`}
                    >
                      {phoneIsNA ? (
                        'N/A'
                    ...
                    <td
                      className={`py-4 pr-6 ${
                        emailIsNA ? 'text-red-600' : 'text-emerald-600'
                      }`}
                    >
                      {emailIsNA ? (
                        'N/A'
      ...
      <div className="mt-6 flex items-center justify-between">
        <div className="text-sm text-slate-600">
          Showing {total === 0 ? 0 : (page - 1) * limit + 1} - {Math.min(total, page * limit)} of{' '}
          {total} results
        </div>
        <div className="flex items-center gap-3">
          <button
